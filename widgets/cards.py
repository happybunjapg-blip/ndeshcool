...
def glass_card(content, padding=16, margin=0, accent=None) -> ft.Container:
    return ft.Container(
        ...
        border=ft.Border.all(1, ft.Colors.with_opacity(0.14, accent) if accent else theme.SURFACE_BORDER),
        shadow=ft.BoxShadow(
            blur_radius=20,
...
def kpi_card(title, value, icon, color, trend: Optional[str] = None, live=False) -> ft.Container:
    ...
    content = ft.Column([
        ...
        ft.Text(value, size=24, weight=ft.FontWeight.W_800, color=ft.Colors.WHITE),
        ft.Text(title, size=12, color=theme.TEXT_DIM),
    ], spacing=6)
    if trend:
...
def stock_card(item, compact: bool = False) -> ft.Container:
    ...
    children = [
        ft.Row([
            ft.Text(item.name, size=13, weight=ft.FontWeight.W_600, color=ft.Colors.WHITE),
        ...
        ft.ProgressBar(value=fill_ratio, color=accent, bgcolor=ft.Colors.with_opacity(0.12, ft.Colors.WHITE),
...
def customer_card(customer, on_click=None) -> ft.Container:
    ...
    return glass_card(
        ft.Row([
            ...
            ft.Column([
                ft.Text(customer.name, size=14, weight=ft.FontWeight.W_600, color=ft.Colors.WHITE),
            ...
            ft.Column([
                ...
                ft.Text("Credit" if customer.is_credit else "Cash", size=10, color=theme.TEXT_DIM),
            ], horizontal_alignment=ft.CrossAxisAlignment.END, spacing=2),
        ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
        padding=12,
        accent=balance_color if customer.balance > 0 else None,
    ) if on_click is None else ft.GestureDetector(
        on_tap=on_click,
        content=glass_card(
            ft.Row([
                ft.Container(
